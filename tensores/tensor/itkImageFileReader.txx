...
// direction matrix
  ...
  // It is exactly the same for our specialised DWI image type
  if( strcmp( output->GetNameOfClass(), "DWImages" ) == 0 ){
	typedef typename TOutputImage::AccessorFunctorType AccessorFunctorType;
	AccessorFunctorType::SetVectorLength( output, m_ImageIO->GetNumberOfComponents() );
  }
...
#define ITK_CONVERT_BUFFER_IF_BLOCK(type)               \
   ...
   {                                                   \
   if( strcmp( this->GetOutput()->GetNameOfClass(), "VectorImage" ) == 0 || strcmp( this->GetOutput()->GetNameOfClass(), "DWImages" ) == 0 ) \
     { \
     ConvertPixelBuffer<                                 \
      type,                                             \
