...
#define roundint(x) ((x)>=0?(long)((x)+0.5):(long)((x)-0.5))
...
//    m_ImageDimension=itkGetStaticConstMacro(SetDimension);
  ...
  ::GenerateData()
  {
    //int next;
	unsigned int k;
    int j,i,d,dist,dist_2,asignacion,icurold,icurnew,icuraux,r,aux_j,rechazar,index,newIndex;
	int l=0;
    int numrechazos=0;
    ...
    std::cout << "Auxiliar " << auxiliar << " sizeof(struct element) " << sizeof(struct element) << std::endl;
    struct element *Element;
    Element=(struct element*)malloc(sizeof(struct element)*auxiliar*1000);
    //Element=(struct element*)malloc(sizeof(struct element)*auxiliar*2*8*m_K);
    ...
    for (k=0;k<m_ImageDimension;k++) {
      CurrentIndex[k] = 0;  
    }

    for (index=0; index < auxiliar; index++) {
      Element[index].DObj.reserve(m_K);
      Element[index].PosObj.reserve(m_K);
      //Element[index].DObj=std::vector<float>(m_K);
      //Element[index].PosObj=std::vector<IndexType>(m_K);
      for(i=0;i<m_K;i++) {
	// Element[index].DObj[l]=0;
	Element[index].DObj[i]=0;
	Element[index].PosObj[i]=CurrentIndex;
      }            
    }

    for (index=0, domainIt.GoToBegin() ; !domainIt.IsAtEnd();  ++domainIt, index++) {           
	...
	while (1){      
	  ...
	  if ( (MapRegion.IsInside(newMapIndex)) && (offsetBucle!=centro) ) {
		...
		dist_2=(int)roundint(distreal_2);
	  ...
	  offsetBucle[0]=-1;
	  for (k=1;k<m_ImageDimension;k++) {
	    offsetBucle[k]=offsetBucle[k]+1;
	    if (offsetBucle[k]!=2) break;
	    offsetBucle[k]=-1;
	  }
