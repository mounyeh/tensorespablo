...
void DTITensor<T>::ComputeEigenValues( EigenValuesArrayType& eig ) const
{
	...
	EigenValuesArrayType eigReverse;

	itk::SymmetricSecondRankTensor<double,3> symTensor;
	for(unsigned i=0; i<6; i++){
		symTensor[i]=(*this)[i];
	}
	
	//ComputeEigenValues( eigenValues );
	symTensor.ComputeEigenValues(eigReverse);
	eig[0] = eigReverse[2];	
	eig[1] = eigReverse[1];	
	eig[2] = eigReverse[0];
...
*/
	...
	itk::Matrix<RealType, 3, 3> eigval;
	eigval.Fill(0.0);
	
	for(unsigned i=0; i<3; i++){
		eigval(i,i)=log(logEigenValues[i]);
	} 	
...
/*
	...
	Matrix<RealType,3,3> logMatrix;
//	logMatrix=eigvec.GetVnlMatrix()*logEigval.GetVnlMatrix()*eigvec.GetTranspose();
	logMatrix=eigvec.GetTranspose()*logEigval.GetVnlMatrix()*eigvec.GetVnlMatrix();
...
//	result[5]=sqrt(2)*logMatrix[1][2];
	result[3]=logMatrix[0][1];
	result[4]=logMatrix[0][2];
	result[5]=logMatrix[1][2];
...
template<class T>
void DTITensor<T>::ComputeEigenSystem( EigenValuesArrayType& eigval, EigenVectorsMatrixType& eigvec ) 
{

	EigenValuesArrayType eigvalReverse;
	EigenVectorsMatrixType eigvecReverse;
	...
	//ComputeEigenValues( eigenValues );
	symTensor.ComputeEigenAnalysis(eigvalReverse, eigvecReverse);

	eigval[0] = eigvalReverse[2];
	eigval[1] = eigvalReverse[1];
	eigval[2] = eigvalReverse[0];

	eigvec[0][0] = eigvecReverse[2][0];
	eigvec[0][1] = eigvecReverse[2][1];
	eigvec[0][2] = eigvecReverse[2][2];
	eigvec[1][0] = eigvecReverse[1][0];
	eigvec[1][1] = eigvecReverse[1][1];
	eigvec[1][2] = eigvecReverse[1][2];
	eigvec[2][0] = eigvecReverse[0][0];
	eigvec[2][1] = eigvecReverse[0][1];
	eigvec[2][2] = eigvecReverse[0][2];
