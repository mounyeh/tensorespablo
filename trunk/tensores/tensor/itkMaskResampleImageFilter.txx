/*=========================================================================
  ...
  Module:    $RCSfile: itkMaskResampleImageFilter.txx,v $
  Language:  C++
...
#ifndef _itkMaskResampleImageFilter_txx
#define _itkMaskResampleImageFilter_txx
 ...
 */
template <class TInputImage, class TOutputImage, class TMaskImage, class TInterpolatorPrecisionType>
MaskResampleImageFilter<TInputImage, TOutputImage,TMaskImage,TInterpolatorPrecisionType>
  ...
  int threadId)
{
  ...
  typedef ImageRegionIteratorWithIndex<TOutputImage> OutputIterator;
	typedef ImageRegionConstIterator<TMaskImage> MaskIterator;

  OutputIterator outIt(outputPtr, outputRegionForThread);
  MaskIterator	 maskIt(this->GetMask(), outputRegionForThread);
  ...
  while ( !outIt.IsAtEnd() )
  {
	if(maskIt.Get())
	{
		index[0]=round(inputIndex[0]);
		index[1]=round(inputIndex[1]);
		index[2]=round(inputIndex[2]);
		outIt.Set(inputPtr->GetPixel(outIt.GetIndex()));
	}else{		
  ...
  int threadId)
{
  ...
  typedef ImageLinearIteratorWithIndex<TMaskImage> MaskIterator;
  ...
  while ( !outIt.IsAtEnd() )
    ...
    while( !outIt.IsAtEndOfLine() )
	  ...
	  if(maskIt.Get())
	  {
		...
		outIt.Set(inputPtr->GetPixel(index));
	  }
	  else
	  {
