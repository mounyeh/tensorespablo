...
typedef enum {CM_NOP, CM_SELECT, CM_BOX, CM_RGB} ClickModeType;
...
*/
const int NUM_ImageModeTypes = 9;
typedef enum {IMG_VAL, IMG_INV, IMG_LOG, IMG_DX, IMG_DY, IMG_DZ,
  IMG_BLEND, IMG_MIP, IMG_COLOR} ImageModeType;
const char ImageModeTypeName[9][8] =
  {{'V', 'a', 'l', 'u', 'e', '\0', ' ', ' '},
  {'I', 'n', 'v', 'e', 'r', 's', 'e', '\0'},
  {'L', 'o', 'g', '\0', ' ', ' ', ' ', ' '},
  {'D', 'e', 'r', 'i', 'v', '-', 'X', '\0'},
...
class ClickPoint 
  ...
  public:
    ClickPoint()
    : x(0),y(0),z(0),value(0),clase(0){}
    ClickPoint( const ClickPoint & p )
    { x = p.x; y = p.y; z = p.z; value = p.value; clase = p.clase; }
    ClickPoint(float _x,float _y,float _z,double v,int c)
    : x(_x),y(_y),z(_z),value(v),clase(c){}
...
template <class imType>
  ...
  protected:
    ...
    float         cClickSelectV;
    void          (* cClickMoveCallBack)(float x,float y,float z);
    void           * cClickMoveArg;
    void          (* cClickMoveArgCallBack)(float x, float y, float z, void *clickMoveArg);
    ...
    void          (* cClickSelectArgCallBack)(float x, float y, float z, 
                                              float v, int c, void *clickSelectArg);
	...
	bool        cViewRAStags;
	bool        cAxial, cSagital, cCoronal;
    float       cOrientationMatrix[3][3];
	...
	unsigned char  *cWinImColorData;
    unsigned short *cWinZBuffer;
    
    double cDataMax, cDataMin;
	double cColorDataMax, cColorDataMin;
  ...
  public:
    ...
    *  various events */
    ...
    virtual void clickMove(float newX, float newY, float newZ);
    void         clickMoveCallBack(
                              void (*newClickMoveCallBack)(float, float,
                              float));
    void         clickMoveCallBack(
                              void (*newClickMoveArgCallBack)(float, float,
                              float,
                              void *),
                              void * newClickMoveArg);
    ...
    void        SetClickedRadius(unsigned int i) {m_radius = i;};
    ...
    */
    ...
    void  clearClickedPointsStored(){cClickedPoints.clear();this->update();}
    
    void  deleteLastClickedPointsStored(){cClickedPoints.pop_front();this->update();}  
    void showClickedPoints(){cViewClickedPoints=true;this->update();}  
    void hideClickedPoints(){cViewClickedPoints=false;this->update();}  
    ...
    void  saveClickedPointsStored() {
      ...
      while( point != cClickedPoints.end() )
        {
        fpoints << point->x << " " << point->y << " " << point->z << " " << point->value << " " << point->clase << std::endl;
        ++point;
        }
      fpoints.close();
      };
    
	std::list<ClickPoint> getClickedPointsStored() {
      return cClickedPoints;
	};

	//void  getClickedPointsStored() {
//      std::list< ClickPoint >::const_iterator point = cClickedPoints.begin();
//      int count = 0;
//	  while( point != cClickedPoints.end() )
//        {
...
//      };
    ...
    void  loadClickedPoints() {    
       float x,y,z,val;
       int clase;
       const char * filename = fl_file_chooser("Please select a file name","*.*","");
       char line[10];
       std::ifstream fpoints( filename );
       cClickedPoints.clear();
       std::list< ClickPoint >::const_iterator point = cClickedPoints.begin();
       if (fpoints.is_open()) {
	 while (! fpoints.eof() ) {
	   fpoints.getline(line,10,' ');
	   x = atof(line);
	   fpoints.getline(line,10,' ');
	   y = atof(line);
	   fpoints.getline(line,10,' ');
	   ...
	   ClickPoint clickedPoint(x, y, z, val, clase);
	...
	void          ViewColorMode(bool newMode) {cViewColorMode = newMode;}
    
    virtual void size(int w, int h);
    virtual void resize(int x, int y, int w, int h);
    virtual void change_activeclass(int c);
  ...
  void viewRAStags(bool value){cViewRAStags = value;}
  bool viewRAStags(){return cViewRAStags;}
  void SetAxial(){cAxial = true;cSagital = false;cCoronal=false;}
  void SetSagital(){cAxial = false;cSagital = true;cCoronal=false;}
  void SetCoronal(){cAxial = false;cSagital = false;cCoronal=true;}
  void SetOrientationMatrix(float *m){
  unsigned int k=0;
  for (unsigned int i=0;i<3;i++) {
    for (unsigned int j=0;j<3;j++) {
	  cOrientationMatrix[i][j] = *(m+k);
      k++;
	}
  } 
  };
...
SliceView<imType>::SliceView(int x, int y, int w, int h, const char * )
  ...
  cClickMoveCallBack = NULL;
  cClickMoveArg = NULL;
  cClickMoveArgCallBack = NULL;
...
SliceView<imType>::~SliceView()
  ...
  if(cWinImData != NULL)
    delete [] cWinImData;
  cWinImData = NULL;
  if(cWinImColorData != NULL)
    delete [] cWinImColorData;
  cWinImColorData = NULL;
  if(cWinZBuffer != NULL)
    delete [] cWinZBuffer;
  cWinZBuffer = NULL;
...
template <class imType>
void SliceView<imType>::
clickMoveCallBack(void 
                    (*newClickMoveCallBack)(float _x, float _y, 
                                              float _z))
  {
  cClickMoveCallBack = newClickMoveCallBack;
  }
...
void SliceView<imType>::
clickMoveCallBack(void 
                    (*newClickMoveArgCallBack)(float _x, float _y, float _z,void *clickMoveArg), 
                    void * newClickMoveArg)
  {
  cClickMoveArg = newClickMoveArg;
  cClickMoveArgCallBack = newClickMoveArgCallBack;
  }

template <class imType>
void SliceView<imType>::
clickMove(float newX, float newY, float newZ)
  {   
    cClickSelect[0] = newX;
  if(cClickSelect[0]<0)
    cClickSelect[0] = 0;
  ...
  if(cClickMoveCallBack != NULL)
    cClickMoveCallBack(cClickSelect[0], cClickSelect[1], 
    cClickSelect[2]);
  if(cClickMoveArgCallBack != NULL)
    cClickMoveArgCallBack(cClickSelect[0], cClickSelect[1], 
    cClickSelect[2],cClickMoveArg);
...
void SliceView<imType>::clickSelect(float newX, float newY, float newZ) {    
  ...
  std::cout << "doing clickSelect" << std::endl;
  if (m_radius > 0) {
  float newClickSelect[3];
  for (int x= -(int)m_radius;x<=(int)m_radius;x++) {
	for (int y= -(int)m_radius;y<=(int)m_radius;y++) {
	  for (int z= -(int)m_radius;z<=(int)m_radius;z++) {
	    if (x*x + y*y + z*z <= (int)m_radius*m_radius) {
        newClickSelect[0] = cClickSelect[0] + x;
		newClickSelect[1] = cClickSelect[1] + y;
		newClickSelect[2] = cClickSelect[2] + z;
		if (newClickSelect[0] >0 && newClickSelect[0] < cDimSize[0] && 
		    newClickSelect[1] >0 && newClickSelect[1] < cDimSize[1] && 
		    newClickSelect[2] >0 && newClickSelect[2] < cDimSize[2]) {
			ind[0] = (unsigned long)newClickSelect[0];
            ind[1] = (unsigned long)newClickSelect[1];
            ind[2] = (unsigned long)newClickSelect[2];
		    cClickSelectV = cImData->GetPixel(ind);
			
            ClickPoint clickedPoint(newClickSelect[0], newClickSelect[1], newClickSelect[2], cClickSelectV, cActiveClass);
            cClickedPoints.push_front( clickedPoint ); 
  
			if(cClickSelectCallBack != NULL)
			cClickSelectCallBack(newClickSelect[0], newClickSelect[1], 
			 newClickSelect[2], cClickSelectV, cActiveClass);
            if(cClickSelectArgCallBack != NULL)
            cClickSelectArgCallBack(newClickSelect[0], newClickSelect[1], 
			    newClickSelect[2], cClickSelectV, cActiveClass,
			    cClickSelectArg);
          }
		 }
    ...
    ClickPoint clickedPoint(cClickSelect[0], cClickSelect[1], cClickSelect[2], cClickSelectV, cActiveClass);
    cClickedPoints.push_front( clickedPoint ); 
  
    if(cClickSelectCallBack != NULL)
     cClickSelectCallBack(cClickSelect[0], cClickSelect[1], 
			 cClickSelect[2], cClickSelectV, cActiveClass);
    if(cClickSelectArgCallBack != NULL)
...
template <class imType>
void SliceView<imType>::change_activeclass(int value)
  {
    cActiveClass = value;
  }
...
int SliceView<imType>::handle(int event)
  ...
  switch(event)
    ...
    case FL_MOVE:
      ...
      if(button <= 0)
        {
        if(cClickMode == CM_SELECT || cClickMode == CM_BOX || cClickMode == CM_RGB) 
          {
          double originX = 0;
          double originY = 0;
          if(cWinZoom<=1)
          ...
          if(cImageMode != IMG_MIP)
            ...
            - cWinMinY)
            * cWinDataSizeX];
          if(cClickMode == CM_SELECT || cClickMode == CM_RGB)  {
            this->clickMove(p[0], p[1], p[2]);
	      } else 
        ...
        if(cClickMode == CM_SELECT || cClickMode == CM_BOX) 
          ...
          if(cClickMode == CM_SELECT)
            this->clickSelect(p[0], p[1], p[2]);
			  // Aqui sustituir clickSelect por el nuevo metodo. 
